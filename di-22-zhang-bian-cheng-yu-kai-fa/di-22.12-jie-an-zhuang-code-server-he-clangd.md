...
# cp -Rf /tmp/root.x86_64/* /compat/linux   # 复制解压后的文件到 FreeBSD Linux 兼容目录
```

## 服务器配置 pacman 源，并添加 archlinuxcn 仓库


编辑 `/compat/linux/etc/pacman.conf`，写入软件源相关配置：
...
编辑 `/etc/resolv.conf` 并复制到 `/compat/linux/etc` 目录中。
...
使用 daemon 后台启动 code-server，并关闭认证，将 PID 写入指定文件：
...
建立 SSH 本地端口转发，将本地 8080 端口映射到远程服务器的 127.0.0.1:8080：
...
回到 code-server，它作为一款 Linux 程序，在尝试访问文件或目录时，默认会先在 `/compat/linux` 下查找。假设你想打开 `/usr/src` 目录以查看 FreeBSD 的源码树，如果 `/compat/linux/usr/src` 存在，那么实际被打开的是 `/compat/linux/usr/src`，而不是你真正想要的 `/usr/src`。这时，我们需要删除 `/compat/linux/usr/src`，确保 FreeBSD 内核能够 fallback 到正确的 `/usr/src`。  
...
* 如何在服务器上通过 HTTPS 提供 code-server 服务
* 探讨 Linux 兼容层与 Linux jail 的区别
